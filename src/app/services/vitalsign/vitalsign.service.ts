...
export class VitalsignService {
  ...
  refreshVitalsigns(patientId: string) {
    ...
    this.fhirClient.search({
      ...
      if (response) {
        ...
        this.dataStore.vitalsigns = this.dataStore.vitalsigns.concat(
          _.filter(response.data.entry, function (o) { return o.resource.resourceType === 'Observation'; })
        );
        ...
        if (response.data.link[1].relation === 'next') {
          // temp fix http to https. to be removed
          response.data.link[1].url = response.data.link[1].url.replace(/http/g, 'https');
  ...
  createVitalsign(vitalsign: any) {
    ...
    this.fhirClient.create({
      ...
      // debugger;
      // the response 201 does not provide the Observation ID. It provides an OperationOutcome resource.
      // Refresh of the entire list is needed.
      // TO BE IMPROVED
